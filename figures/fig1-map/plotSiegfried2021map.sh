...
h5lakes='../../data/outlines/SiegfriedFricker2018-outlines.h5'
...
h5ls ${h5lakes} | grep "Group" | awk '{print $1}' > groups
while read g
do
    # dump x and y values to separate files
    printf '.'
    h5dump -d /${g}/x -y -A 0 -b LE -o tmpx.bin -O ${h5lakes}
    h5dump -d /${g}/y -y -A 0 -b LE -o tmpy.bin -O ${h5lakes}
    ...
    gmt psxy lake.xy $map $reg -O -K -Wthick,blue -Gblue >> $out.ps
